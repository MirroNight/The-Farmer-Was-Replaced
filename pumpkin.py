...
def check_and_harvest_pumpkin(arr, x, y, size, xi=0, yi=0, mv=True, fert=False):
...
def subregions_pumpkin(arr, x, y, size=get_world_size(), fert=False):
    # quick_print('----- -----')
    ...
    if arr[x][y] <= 0:
        ...
        arr = check_and_harvest_pumpkin(arr, x, y, size, 0, 0, True, fert)
        if arr[x][y] < 0:
            sub_ori = -1
        else:
            sub_ori = 1
    ...
    for xi in range(size):
        for yi in range(size):
            ...
            if can_harvest():
                arr[x+xi][y+yi] = 1
                arr[x][y] += 1*sub_ori
                arr = check_and_harvest_pumpkin(arr, x, y, size, xi, yi, False, fert)
...
def remainder_pumpkin(arr, size, fert):
    ...
    if remainder2:
        ...
        for xi in range(start, get_world_size(), remainder2):
            for _ in range(0, get_world_size(), remainder2):
                arr = subregions_pumpkin(arr, xi, 0, remainder2, size, fert)
        
        # bottom left 1st remainder
        for y in range(remainder2, get_world_size(), remainder1):
            arr = subregions_pumpkin(arr, start, y, remainder1, size, fert)

        # top left 2nd remainder
        for yi in range(start, get_world_size(), remainder2):
            for _ in range(0, get_world_size(), remainder2):
                arr = subregions_pumpkin(arr, 0, yi, remainder2, size, fert)
        
        # top left 1st remainder
        for x in range(remainder2, get_world_size(), remainder1):
            arr = subregions_pumpkin(arr, x, start, remainder1, size, fert)
    
    return arr


def main_pumpkin(arr, size, fert=False):
    ...
    for x in range(get_world_size() // size):
        for y in range(get_world_size() // size):
            arr = subregions_pumpkin(arr, x*size, y*size, size, fert)

    if get_world_size() % size:
        arr = remainder_pumpkin(arr, size, fert)
...
def restock_pumpkin(arr, init=False, size=get_world_size(), fert=False):
    plant_func1 = utils.get_plant_func(Grounds.Soil, Entities.Pumpkin, 0.0, False, False, fert)
    plant_pattern = patterns.get_pattern_func(Items.Pumpkin, plant_func1)
    
    if init:
        plant_pattern()
    else:
        arr = main_pumpkin(arr, size, fert)
    return arr
